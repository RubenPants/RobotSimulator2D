...
from population.utils.network_util.activations import identity_activation, tanh_activation
...
class AdaptiveLinearNet:
    ...
    def reset(self):
        with torch.no_grad():
            self.input_to_output = (self
                                    .get_init_weights(self.input_coords, self.output_coords, self.delta_w_node)
                                    .unsqueeze(0)
    ...
    def activate(self, inputs):
        ...
        with torch.no_grad():
            ...
            delta_w = self.cppn_activation(self.delta_w_node(
                    x_out=x_out,
                    y_out=y_out,
                    x_in=x_in,
                    y_in=y_in,
